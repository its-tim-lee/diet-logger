...
const DatePickerModal: React.FC<DatePickerModalProps> = ({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`relative w-full max-w-md bg-[#1f2e18] rounded-t-[2rem] p-6 shadow-2xl transform transition-transform duration-300 ease-out flex flex-col gap-6 ${
      ...
      >
        ...
        <div className="grid grid-cols-7 text-center gap-1">
          {["S", "M", "T", "W", "T", "F", "S"].map((d) => (
            <div key={d} className="text-xs text-gray-500 font-bold py-2">
              {d}
            </div>
          ))}
          {blanks.map((b) => (
